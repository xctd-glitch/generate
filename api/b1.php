...
function get_branch_io($url, $access_token, $bit_tkn, $pick, $fb) {
	...
	return branch_result($connectURL, $payload, $bit_tkn, $pick, $fb);
}
function branch_result($url, $load, $bit_tkn, $pick, $fb) {
	$ch = curl_init();
	$timeout = 15;
	...
	//return $short;
	return get_bitly_short_url($short, $bit_tkn, $pick, $fb);
}


function get_bitly_short_url($short, $bit_tkn, $pick, $fb) {
    if($pick == '1_jmp'){
    $app = 'https://api-ssl.bitly.com/v3/shorten?longUrl='.urlencode(trim(preg_replace('/\s\s+/', ' ', $short))).'&access_token='.$bit_tkn.'&domain=j.mp';
    }
    else if($pick === '2_shr'){
    $url = 'https://www.shareaholic.com/v2/share/shorten_link?url='.$short.'&service=shrlc';
    $ch = curl_init($url);
    ...
    if(empty($fb)){
        ...
        //$short = "https://l.facebook.com/l.php?u=".$array["data"]."&h=".substr(str_shuffle("abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789") , 0 , 7)."&s=1";
        $short = "https://l.facebook.com/l.php?u=".$array["data"]."&h=".substr(str_shuffle("abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789") , 0 , 7)."&s=1";
    }
    return $short;
    ...
    $url = 'https://bom.to/api/?key=USRUxH0FzrTJ&url='.$short;
    $ch = curl_init($url);
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
    curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, false);
    curl_setopt($ch, CURLOPT_HEADER, 0);
    ...
    if(empty($fb)){
        ...
        //$short = "https://l.facebook.com/l.php?u=".$array["data"]."&h=".substr(str_shuffle("abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789") , 0 , 7)."&s=1";
        $short = "https://l.facebook.com/l.php?u=".$array["short"]."&h=".substr(str_shuffle("abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789") , 0 , 7)."&s=1";
    }
    return $short;
    ...
    $app = 'https://api-ssl.bitly.com/v3/shorten?longUrl='.urlencode(trim(preg_replace('/\s\s+/', ' ', $short))).'&access_token='.$bit_tkn;
    }
	$connectURL = $app;
	return curl_get_result($connectURL, $fb);
}
function curl_get_result($url, $fb) {
	$ch = curl_init();
	$timeout = 15;
	curl_setopt($ch,CURLOPT_URL,$url);
	curl_setopt($ch,CURLOPT_RETURNTRANSFER,1);
    ...
    if(empty($fb)){
        ...
        //$short = "https://l.facebook.com/l.php?u=".$response->data->url."&h=".substr(str_shuffle("abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789") , 0 , 7)."&s=1";
        $short = "https://l.facebook.com/l.php?u=".$response->data->url."&h=".substr(str_shuffle("abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789") , 0 , 7)."&s=1";
    }
	return $short;
...
$str_build = array(randomName(), base64url_encode(substr(str_shuffle('abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789') , 0 , 5).','.strtoupper($sub_id).','.time().','.$canonical_url.','.$net));
$url_ex = str_replace($str_param, $str_build, htmlspecialchars_decode($genurl));
    ...
    foreach ($textAr as $line){
        echo get_branch_io($line, $branch_key, $bit_key, $b_pick, $fb_pick) . ',';
